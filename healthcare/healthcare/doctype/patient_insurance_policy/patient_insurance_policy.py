...
class PatientInsurancePolicy(Document):
	...
	def validate_expiry_date(self):
		if getdate(self.policy_expiry_date) < getdate():
			frappe.throw(_("Expiry Date for the Insurance Policy cannot be a past date"))

	def validate_policy_overlap(self):
		insurance_policy = frappe.db.exists(
		...
		if insurance_policy:
			frappe.throw(
				_(
					"Patient {0} already has an active insurance policy {1} with the coverage plan {2} for this period"
				).format(
					frappe.bold(self.patient),
					get_link_to_form("Patient Insurance Policy", insurance_policy),
					frappe.bold(self.insurance_plan),
				),
				title=_("Duplicate"),
			)
	...
	def validate_policy_number(self):
		insurance_policy = frappe.db.exists(
			"Patient Insurance Policy",
			{"patient": self.patient, "docstatus": 1, "policy_number": self.policy_number},
		)
		if insurance_policy:
			frappe.throw(
				_("Patient {0} already has an Insurance Policy {1} with the same Policy Number {2}").format(
					frappe.bold(self.patient),
					get_link_to_form("Patient Insurance Policy", insurance_policy),
...
def get_insurance_price_lists(insurance_policy, company):
	...
	"""
	price_lists = {}
	insurance_plan, insurance_payor = frappe.db.get_value(
	...
	if insurance_payor and company:
		default_price_list = frappe.db.get_value(
			...
			{"insurance_payor": insurance_payor, "company": company},
			"default_price_list",
		)
