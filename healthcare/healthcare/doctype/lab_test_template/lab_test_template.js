...
frappe.ui.form.on("Lab Test Template", {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Change Template Code"), function () {
	...
	item: function (frm) {
		if (frm.doc.item) {
			frappe.db.get_value("Item", frm.doc.item, ["item_group"]).then(r => {
				frm.set_value({
...
let change_template_code = function (frm) {
		...
		primary_action: function () {
			...
			if (values) {
				frappe.call({
					...
					args: { lab_test_code: values.lab_test_code, doc: frm.doc },
	...
	d.set_values({
		lab_test_code: frm.doc.link_existing_item
			? frm.doc.item
			: frm.doc.lab_test_code,
	});
};
